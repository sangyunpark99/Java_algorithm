...
public class Main {
    ...
    public static int go(int here, int visited, int capacity) { // 가방 idx, 보석 방문(비트 마스킹), 남은 용량
        ...
        for (int i = 0; i < n; i++) {
            if (((1 << i) & visited) > 0) { // 이미 방문한 보석인 경우
                continue;
            }

            if (capacity >= jewels[i]) { // 가방에 보석을 담을 수 있는 경우 - 보석을 담는 경우
                dp[here][visited][capacity] = Math.max(dp[here][visited][capacity],
                        go(here, visited | (1 << i), capacity - jewels[i]) + 1);
            } else { // 가방에 보석을 담을 수 없는 경우 - 보석을 담지 않는 경우
